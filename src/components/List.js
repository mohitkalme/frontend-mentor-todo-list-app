...
export default function List( { task,deleteTask,toggleTaskCompleted } ) {

  const y = useMotionValue(0);
  const boxShadow = useRaisedShadow(y);
    
  return (
    <Reorder.Item value={task} id={task.id} style={{ boxShadow, y }}>
    ...
    >
      ...
      <div id="cross" className="block sm:hidden"  onClick={() =>deleteTask(task.id)}>
